...
def get_batch_customers(request: Request, realm_id: str) -> list[dict]:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                select
                    ...
                    qb_customer_name,
                    avsight_customer_name,
                    active
                from public.qb_customer
                where realm_id = %s
                  and include_in_batch_reports = true
                order by avsight_customer_name nulls last, qb_customer_name
                """,
                (realm_id,)
            )
    ...
    for r in rows:
        customers.append({
            "quickbooks_customer_id": str(r.get("quickbooks_customer_id", "")).strip(),
            "qb_customer_name": str(r.get("qb_customer_name", "") or "").strip(),
            "avsight_customer_name": str(r.get("avsight_customer_name", "") or "").strip(),
            "active": bool(r.get("active", True)),
        })

    return [c for c in customers if c["quickbooks_customer_id"]]
...
def compare_invoice_lines_summary_year_vs_year_batch(
    ...
    customers = get_batch_customers(request=request, realm_id=realmId)
